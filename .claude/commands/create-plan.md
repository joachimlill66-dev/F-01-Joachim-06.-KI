...
- Erstelle den Plan im `05_plans/`-Verzeichnis mit Dateiname: `YYYY-MM-DD-{beschreibender-name}.md`
...
1. **Kern-Referenzdateien lesen:**
   ...
   - `01_context/` — Hintergrund-Kontext über den User und das Projekt

2. **Relevante Bereiche erkunden:**
   - Bei Command-Erstellung: bestehende Commands in `.claude/commands/` lesen
   - Bei Output-Änderungen: `09_Marktueberblick/`-Struktur und Beispiele erkunden
   - Bei Template-Updates: `04_reference/` auf bestehende Patterns prüfen
   - Bei Skript-Erstellung: `03_scripts/` auf Konventionen prüfen
...
3. Gib den vollständigen Pfad zur Plan-Datei an: `05_plans/YYYY-MM-DD-{name}.md`
4. Erinnere den User daran, `/implement 05_plans/YYYY-MM-DD-{name}.md` zur Umsetzung auszuführen
